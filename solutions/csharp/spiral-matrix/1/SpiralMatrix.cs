public static class SpiralMatrix
{
    ...
    public static int[,] GetMatrix(int size)
    {
        ...
        for (var i = 1; i <= size * size; i++)
        {
            ...
            if (!spiralMatrix.IsValid(tx, ty) || spiralMatrix[tx, ty] != 0)
            {
    ...
    private static bool IsValid<T>(this T[,] array, int i, int j) => i >= 0 && j >= 0 && i < array.GetLength(0) && j < array.GetLength(1);
}
